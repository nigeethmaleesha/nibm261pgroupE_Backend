...
const sendOtpEmail = async ({ email, otp, purpose, expiresInMinutes }) => {
  ...
  const isRegistration = normalizedPurpose === 'REGISTER';
  const actionLabel = isRegistration ? 'registration' : 'login';
  const subject = `RepairFlow - ${isRegistration ? 'Registration' : 'Login'} OTP`;
  const fromName = process.env.EMAIL_FROM_NAME || 'RepairFlow';

  const text = [
    `Your RepairFlow ${actionLabel} verification code is ${otp}.`,
    `This code expires in ${expiresInMinutes} minute${Number(expiresInMinutes) === 1 ? '' : 's'}.`,
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;max-width:560px;margin:auto">
      <h2 style="margin-bottom:8px">RepairFlow ${isRegistration ? 'Registration' : 'Login'} Verification</h2>
      ...
      <p>This code expires in <strong>${expiresInMinutes} minute${Number(expiresInMinutes) === 1 ? '' : 's'}</strong>.</p>
